        ...
        for i in range(len(branch_data['skeleton-id'])):
            if not branch_data['branch-type'][i]==2:
                ...
                if dist < 20:
                    ...
                    col = branch_data['image-coord-dst-1'][i]
                    row = branch_data['image-coord-dst-0'][i]
                    if not ([row,col,row2,col2] in remove) and ([row2,col2,row,col] in remove):
...
def generate_report(pathscan,for_report = True,run_times=2,output_folder=None):
        ...
        for i in range(len(counts)-1,-1,-1):
            if counts[i]>400:
                if bins[i+1]>2000:
                    ...
                    print("Type 1")
                    teeth_mask=ts.prediction_teeth(scan,250,1500) #250,1500
                    #plot_3d(teeth_mask,0.5)
    ...
    if not for_report:
        dcmData = pydicom.dcmread(r"dummy.dcm")
    ...
    if (for_report):
        ...
        print('Dicoms saved')
    return output_list, classUID,dist
...
def predictions(input_folder,output_folder):
    output_list, classUID,dist = generate_report(input_folder,False,3,output_folder)
